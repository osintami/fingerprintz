...
type Account struct {
	gorm.Model  `json:"-"`
	Name        string
	Email       string `gorm:"index:idx_account_email,unique"`
	ApiKey      string
	Tokens      int
	Role        string
...
func (x *Accounts) WelcomeEmail(account *Account) error {
	...
	if err == nil {
		content := string(body)
		content = strings.ReplaceAll(content, "$apikey", account.ApiKey)
		content = strings.ReplaceAll(content, "$email", account.Email)
		content = strings.ReplaceAll(content, "$name", account.Name)
		//body = strings.ReplaceAll(body, "$tokens", strconv.FormatInt(int64(user.Tokens), 10))
		...
		for _, recipient := range recipients {
			...
			err = x.mail.Send(subject, content, internal, false)
			if err != nil {
				log.Error().Err(err).Str("component", "welcome").Str("email", internal.String()).Msg("email send failed")
			}
			err = x.mail.Send(subject, content, buyer, false)
			if err != nil {
